...
def send_report_email(title: str, content: str) -> tuple[bool, str]:
    ...
    for line in lines:
        ...
        if not stripped:
            ...
            html_lines.append(f'<h3 style="color:#2c3e50;border-bottom:1px solid #eee;padding-bottom:6px;">{stripped[4:]}</h3>')
        elif stripped.startswith("## "):
            html_lines.append(f'<h2 style="color:#1a252f;border-bottom:2px solid #3498db;padding-bottom:8px;">{stripped[3:]}</h2>')
        elif stripped.startswith("# "):
            html_lines.append(f'<h1 style="color:#1a252f;">{stripped[2:]}</h1>')
            ...
            processed = _re.sub(r'\*\*(.+?)\*\*', r'<strong>\1</strong>', stripped)
            # Links
            processed = _re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'<a href="\2" style="color:#3498db;">\1</a>', processed)
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:680px;margin:0 auto;padding:20px;color:#333;">
  <div style="background:linear-gradient(135deg,#1a252f,#2c3e50);color:white;padding:24px 30px;border-radius:12px 12px 0 0;">
    <h1 style="margin:0;font-size:22px;">📊 {title}</h1>
    <p style="margin:8px 0 0;opacity:0.8;font-size:14px;">Ashley Daily Report &mdash; {date_str}</p>
  </div>
  <div style="background:#fff;border:1px solid #e0e0e0;border-top:none;padding:24px 30px;border-radius:0 0 12px 12px;">
    {body_html}
  </div>
  <p style="text-align:center;color:#999;font-size:12px;margin-top:16px;">Generated by Ashley &bull; Personal AI Assistant</p>
</body>
</html>"""
...
class RouterHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/email-report":
            title = str(payload.get("title", "Ashley Report")).strip()
            content = str(payload.get("content", "")).strip()
            if not content:
                self._json_response(400, {"ok": False, "error": "content required"})
                return
            ok, reply = send_report_email(title, content)
            self._json_response(200 if ok else 500, {"ok": ok, "reply": reply})
            return
